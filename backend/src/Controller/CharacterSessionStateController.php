...
final class CharacterSessionStateController extends AbstractController
{
    ...
    public function list(
        ...
        $this->denyAccessUnlessGranted( CampaignVoter::MANAGE, $gameSession->getCampaign(), );

        $states = $stateRepository->findBy( ['gameSession' => $gameSession], ['id' => 'ASC'], );

        return $this->json([ 'states' => array_map( fn (CharacterSessionState $state): array => $this->sessionStateSerializer->serialize($state, true), $states, ), ]);
    }
    ...
    public function create(
        ...
        if (!$character instanceof Character) {
            return $this->json( ['message' => 'Personnage introuvable.'], Response::HTTP_NOT_FOUND, );
        }

        $this->denyAccessUnlessGranted( CampaignVoter::MANAGE, $gameSession->getCampaign(), );

        if ( $gameSession->getCampaign()->getId() !== $character->getCampaign()->getId() ) {
            return $this->json( ['message' => 'Le personnage ne fait pas partie de cette campagne.'], Response::HTTP_BAD_REQUEST, );
        }

        $existingState = $stateRepository->findOneBy([ 'gameSession' => $gameSession, 'character' => $character, ]);

        if ($existingState instanceof CharacterSessionState) {
            $existingState->setParticipating(true);
            $entityManager->flush();

            return $this->json( $this->sessionStateSerializer->serialize($existingState, true), Response::HTTP_OK, );
        }
        ...
        $state = new CharacterSessionState( $gameSession, $character, $initialState, );

        $entityManager->persist($state);
        $entityManager->flush();

        return $this->json( $this->sessionStateSerializer->serialize($state, true), Response::HTTP_CREATED, );
    }
    ...
    public function remove(
        ...
        $state = $stateRepository->findOneBy([ 'gameSession' => $gameSession, 'character' => $character, ]);

        if (!$state instanceof CharacterSessionState) {
            return $this->json( ['message' => 'Ce personnage ne participe pas à cette session.'], Response::HTTP_NOT_FOUND, );
        }
        ...
        return $this->json( $this->sessionStateSerializer->serialize($state, true), );
    }
    ...
    public function adjustMaximumHitPoints(
        ...
        $sessionState = $stateRepository->findOneBy([ 'gameSession' => $gameSession, 'character' => $character, ]);

        if ( !$sessionState instanceof CharacterSessionState || !$sessionState->isParticipating() ) {
            return $this->json( ['message' => 'Ce personnage ne participe pas à cette session.'], Response::HTTP_NOT_FOUND, );
        }
        ...
        if (!is_int($delta) || $delta === 0) {
            return $this->json( ['message' => 'La propriété "delta" doit être un entier non nul.'], Response::HTTP_UNPROCESSABLE_ENTITY, );
        }

        try {
            $newState = $hitPointStateService->adjustMaximum( $character, $sessionState->getState(), $delta, );
        ...
        return $this->json( $this->sessionStateSerializer->serialize($sessionState, true), );
    }
...
public function terminateActiveEffect(
    ...
    if (!$effect instanceof CharacterActiveEffect) {
        return $this->json( ['message' => 'Effet actif introuvable.'], Response::HTTP_NOT_FOUND, );
    }

    $this->denyAccessUnlessGranted( CampaignVoter::MANAGE, $gameSession->getCampaign(), );

    if ( $character->getCampaign()->getId() !== $gameSession->getCampaign()->getId() ) {
        return $this->json( ['message' => 'Le personnage ne fait pas partie de cette campagne.'], Response::HTTP_BAD_REQUEST, );
    }

    if ( $effect->getTargetCharacter()->getId() !== $character->getId() ) {
        return $this->json( ['message' => 'Cet effet n’appartient pas à ce personnage.'], Response::HTTP_BAD_REQUEST, );
    }

    $sessionState = $stateRepository->findOneBy([ 'gameSession' => $gameSession, 'character' => $character, ]);

    if (!$sessionState instanceof CharacterSessionState) {
        return $this->json( ['message' => 'État de session du personnage introuvable.'], Response::HTTP_NOT_FOUND, );
    }
    ...
    public function updateLevelUpPermission(
        ...
        if (!is_bool($allowed)) {
            return $this->json( ['message' => 'La propriété "allowed" doit être un booléen.'], Response::HTTP_BAD_REQUEST, );
        }
    ...
    public function updateProgression(
        ...
        if (!$character->hasProgression($progression)) {
            return $this->json( ['message' => 'Le personnage ne possède pas cette progression.'], Response::HTTP_NOT_FOUND, );
        }
        ...
        if (!is_int($currentValue)) {
            return $this->json( ['message' => 'La propriété "currentValue" doit être un entier.'], Response::HTTP_BAD_REQUEST, );
        }

        if ($currentValue < $progression->getMinimumValue()) {
            return $this->json( ['message' => 'La valeur est inférieure au minimum de la progression.'], Response::HTTP_UNPROCESSABLE_ENTITY, );
        }

        if ( $progression->getMaximumValue() !== null && $currentValue > $progression->getMaximumValue() ) {
            return $this->json( ['message' => 'La valeur dépasse le maximum de la progression.'], Response::HTTP_UNPROCESSABLE_ENTITY, );
        }
        ...
        foreach ($progressions as &$progressionState) {
            if ( ($progressionState['id'] ?? null) !== $progression->getSlug() ) {
                continue;
            }
    ...
    public function showPublic( string $accessToken, PlayerCharacterAccess $playerAccess, ): JsonResponse {
        $state = $playerAccess->requireParticipating($accessToken);

        return $this->json($this->sessionStateSerializer->serialize($state));
    }
    ...
    public function updatePublic(
        ...
        if (!is_array($newState)) {
            return $this->json( ['message' => 'La propriété "state" doit être un objet JSON.'], Response::HTTP_BAD_REQUEST, );
        }
